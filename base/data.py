...
class EEGDataset(Dataset):
    def __init__(self, config, mode):
        ...
        if config['data']['blur_type']['target'].rsplit('.',1)[-1] == 'UniformBlur':
            data_dir = os.path.join(self.data_dir,'../Image_feature',f"{config['data']['blur_type']['target'].rsplit('.',1)[-1]}_{config['data']['blur_type']['params']['blur_kernel_size']}")
        else:
        ...
        features_filename = os.path.join(data_dir, f"{image_feature_name}_{mode}.pt")
